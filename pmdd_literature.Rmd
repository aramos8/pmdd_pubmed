...
articles_year |>
  summarize(with_dates = n_distinct(case_when(is.na(pub_date) == FALSE  ~ pubmed_id)),
            without_dates = n_distinct(case_when(is.na(pub_date) ~ pubmed_id)),
            all = n_distinct(pubmed_id)
            )

```
First, let's keep in mind that out of the 1334 PMDD articles in PubMed, only ~41% of these articles have a value for their publication date. 

```{r}
#Let's get a list of the pubmed_ids that are missing a publication date. 
no_date_articles <- articles_year |>
  dplyr::filter(is.na(pub_date)) |>
  select(pubmed_id)

n_articles_year <- articles_year |>
  group_by(pub_year) |>
  dplyr::filter(is.na(pub_date) == FALSE) |>
  summarize(articles = n_distinct(pubmed_id))
```

```{r}
ggplot(data = n_articles_year) +
  geom_col(mapping = aes(x = pub_year, y = articles))
```
...
ggplot(data = n_articles_institution) +
  geom_col(mapping = aes(x = author_affiliation, y = articles))
```
