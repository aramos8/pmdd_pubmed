...
*Journal style introduction to PMDD*.  
- What is PMDD?\
...
This notebook will use a combination of `DuckDB` and `tidyverse` packages to process the data. Let's first establish a connection to the `pmdd.db` database with the data imported from PubMed using `pmdd_entrez.sh`.Once connected to the database, let's load the "entrez_clean" table into a dataframe. Since we will not need to load any more data from the database in this notebook, we will now close the connection to `pmdd.db`.
...
entrez_clean %>%
  ...
  ggplot(., aes(x = publication_year, y = publications, fill = toupper(mesh_terms))) + geom_col(position = position_dodge()) + xlab("Publication Year") + ylab("Publications using MeSH term") + scale_fill_discrete(name = "MeSH Term")
```
