...
About 66% of the articles extracted from PubMed include keywords. Interestingly, many of the publications that to not have keywords reported in PubMed do have them in PMC. Therefore, a potential future direction of this work could include extracting all the keywords from the PMC record in NCBI. However, Entrez does not have access to PMC keywords, so a web scraper will be used instead, as keywords from PMC articles can be pulled from the `class=kwd_text` HTML element.

First, a list of PMC IDs is collected. 

```{r}
# Get all PMC IDs to extract keywords
kw_pmcids <- entrez_clean %>% 
  filter(keywords == 'NULL') %>% 
  select(pmc_id) %>% 
  filter(!is.na(pmc_id)) %>% 
  mutate(link = paste0("https://www.ncbi.nlm.nih.gov/pmc/articles/", pmc_id, "/")) %>% 
  unique()
  #filter(pmc_id %in% c('PMC6741762', 'PMC7220159'))
         

# 
# read_html("https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6741762/") %>% 
#   html_elements(".kwd-text")
# 
# test <-  get_html_kws(kw_pmcids$link[[1]])
# print(test)

# Write function to get keywords from HTML
get_html_kws <- function(x) {
  read_html((x)) %>% 
    html_elements(".kwd-text") %>% 
    html_text()
}
...
# Get all keywords from HTML
for (i in 1:length(kw_pmcids$link)) {
  GET(kw_pmcids$link[[i]], add_headers('user-agent' ="Bioinformatics student (anaramos.data@gmail.com)")) # Without this line, I get 'Error in open.connection(x, "rb") : HTTP error 403.'
  kw_pmcids$p_keywords[[i]] <- get_html_kws(kw_pmcids$link[[i]])
}
...
Remaining calculations to include:
- Number of affiliations cited?
