...
First, the file lists obtained from PMC were combined into a single CSV file to have all the information required. This was done in a separate R script named `pmc_filelist.R`.
...
#kws_pmc_ids <- c("PMC5533120", "PMC5427117")

# Function to get keywords from article XML
pmc_kws <- purrr::map_df(kws_pmc_ids, function(id) {
  return(data.frame(pmc_id = id, 
                    keywords = xml_find_all(read_xml(paste0('/Volumes/Vault/PMC/PMC_files/', id, ".xml")), "//kwd") %>% xml_text()
                      ))
})


#------------------------------------------------
# This function could have worked too, but I wanted to try map_df
# Also, map_df required substantially fewer lines of code

# get_keywords <-  function(pmc_id) {
#   #xml <- read_xml(paste0("/Volumes/Vault/PMC/PMC_files/", pmc_id))
# 
#   xml <- read_xml(paste0('/Users/anaelizondo-ramos/Downloads/test/oa_comm_xml.incr.2024-06-19/PMC005xxxxxx/',pmc_id, ".xml"))
# 
#   pmc_id <-  pmc_id
# 
#   if(length(xml_find_all(xml, "//kwd") %>% xml_text()) > 0){
#     kw <- list(xml_find_all(xml, "//kwd") %>% xml_text())
#   }else{
#     kw <- 'N/A'
#   }
# 
#   cbind(pmc_id=pmc_id, kw=kw)
# }
# 
# pmc_kws <- as.data.frame(do.call("rbind", lapply(kws_pmc_ids, get_keywords)), stringsAsFactors = FALSE)
#------------------------------------------------
...
citations %>% 
  ...
  ggtitle("Top 5 most cited PMDD publications")

```
The graph above shows the top 5 most cited PMDD publications (among PMDD scientific literature).
